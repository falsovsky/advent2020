...
def forward2(current, waypoint, units):
    ...
    if waypoint[0][0] > 0: w1 = 'E'
    else: w1 = 'W'

    if waypoint[1][0] > 0: w2 = 'N'
    else: w2 = 'S'

    for i in range(0, units):
        new[2] = w1
        new = forward(new, waypoint[0][0])
        new[2] = w2
        new = forward(new, waypoint[1][0])
...
for i in nav:
    ...
    if action == "F":
        ...
        for idx in range(0, 2):
            if move[idx] > 0:
                waypoint[idx][0] += units
            elif move[idx] < 0:
                waypoint[idx][0] -= units

    print("POST", current, "\t", waypoint)
    print()
